...
var selectedEstimatedTurnOverDropDown={"id":"0","name": appSelectEstimatedTurnOver}.obs;

var selectedEmployeesRangeDropDown={"id":"0","name": appSelectIncorporateYear}.obs;
var selectedCompanyRangeDropDown={"id":"0","name": appSelectCompanySizeRange}.obs;
...
//var turnoverListData=TurnOverListModel().obs;


updateProfileBottomSheet(context,{required CompanyAllDetailsData companyAllDetails,
  required String screenName,required  CompanyUserDetailsModel companyUserDetails,required List<TurnOverListData>
  turnOverModelDetails,required List<CompanySizeList> companySizeDetails
  , int index=0}) {

  updateProfileData(context, companyUserDetails: companyUserDetails,turnOverModelDetails: turnOverModelDetails,companySizeDetails:companySizeDetails,
      index: index);
          ...
          builder: (ctx){
            ...
            return Wrap(
              children: <Widget>[
                ...
                children: [
                  commonTextField(
                    controller: aboutaiCompanyController,
                    hintText: "AI Rewritten Text",
                    maxLine: 4,
                  ),
            ...
            onTap: () => rewriteAboutWithAIAPI
                                            ...
                                            child: Row(
                                              children: <Widget>[
                                                ...
                                                SizedBox(height: 5,),
                                                Obx((){
                                                  var industrytype=companyAllDetails.data?.industryList??[];

                                                  return industrytype!=null&&industrytype.isNotEmpty?customDropDown(
                                                      hintText: appSelectDesignation,
                                                      item: [{"id":"0","name": appSelectIndustryType},
                                                        ...industrytype.map((data)=>{'id':data.id,'name':data.name}).toList()??[]
                                                      ],
                                                      selectedValue: industrytype.any((datum)=>datum.id==selectedIndustryTypeDropDown["id"])?selectedIndustryTypeDropDown:{"id":"0","name": appSelectIndustryType},
                                                ...
                                                Obx(() {
                                                  ...
                                                  final List<Map<String, String>> yearList = List.generate(
                                                    currentYear - 1900 + 1,
                                                        (index) {
                                                      final year = (1900 + index).toString();
                                                      return {'id': year, 'name': year};
                                                    },
                                                  );

                                                  return customDropDown(
                                                    hintText: appSelectIncorporateYear,
                                                    item: [
                                                      {"id": "0", "name": appSelectIncorporateYear},
                                                      ...yearList
                                                    ],
                                                    selectedValue: yearList.any((datum) =>
                                                    datum["id"] == selectedInCorporationYearDropDown["id"])
                                                    ...
                                                    onChanged: (Map<String, dynamic>? selectedData) {
                                                      if (selectedData != null) {
                                                        selectedInCorporationYearDropDown.value = {
                                                          ...
                                                          "name": selectedData['name'].toString()
                                                        };
                                                      }
                                                    },
                                                    icon: appDropDownIcon,
                                                ...
                                                SizedBox(height: 5,),
                                                Obx(() {
                                                  var turnoverList = turnOverModelDetails ?? [];

                                                  return turnoverList.isNotEmpty
                                                      ? customDropDown(
                                                    item: [
                                                      {"id": "0", "name": appSelectEstimatedTurnOver},
                                                      ...turnoverList.map((data) => {
                                                        'id': data.id.toString(),
                                                        'name': data.name.toString(),
                                                      }),
                                                    ],
                                                    selectedValue: turnoverList.any((item) =>
                                                    item.id.toString() ==
                                                        selectedEstimatedTurnOverDropDown["id"])
                                                    ...
                                                    icon: appDropDownIcon,
                                                    hintText: appSelectEstimatedTurnOver,
                                                  )
                                                ...
                                                ///Employee range
                                                commonTextFieldTitle(headerName: appSelectCompanySizeRange,isMendatory: false),
                                                SizedBox(height: 5,),
                                                Obx((){

                                                  var companyRange=companySizeDetails??[];

                                                  return companyRange!=null&&companyRange.isNotEmpty?customDropDown(
                                                      hintText: appSelectDesignation,
                                                      item: [{"id":"0","name": appSelectCompanySizeRange},
                                                        ...companyRange.map((data)=>{'id':data.id,'name':data.name}).toList()??[]
                                                      ],
                                                      selectedValue: companyRange.any((datum)=>datum.id==selectedCompanyRangeDropDown["id"])?selectedCompanyRangeDropDown:{"id":"0","name": appSelectCompanySizeRange},
                                                      onChanged: (Map<String,dynamic>? selectedData){
                                                        if(selectedCompanyRangeDropDown!=null){
                                                          selectedCompanyRangeDropDown.value={
                                                            "id": selectedData?['id'].toString() ?? "0",
                                                            "name": selectedData?['name'].toString() ?? appSelectCompanySizeRange
                                                          };
                                                        }
                                                      },
                                                      icon: appDropDownIcon
...
updateProfileData(context,{required  CompanyUserDetailsModel companyUserDetails,int index=0,
  required List<TurnOverListData> turnOverModelDetails, required List<CompanySizeList> companySizeDetails}){
  progressDialog.show();
